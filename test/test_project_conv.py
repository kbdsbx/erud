...
def no_test_conv () :
    ...
    g = erud.nous(
        ...
        X:(1080, 64, 64, 3) ->
            ...
            conv2d(1, 2) W1:xavier_cnn(4, 4, 3, 8) -> relu -> max_pool(8, 8, 8) ->
            ##### 二层卷积
            ##### (1080, 8, 8, 8) -> (1080, 8, 8, 16) -> (1080, 2, 2, 16)
            conv2d(1, 1) W2:xavier_cnn(2, 2, 8, 16) -> relu -> max_pool(4, 4, 4) ->
...
def no_test_conv_with_save_and_load () :
    ...
    # 缓存文件
    path = __file__[:__file__.rfind('\\')]
    filename = path + '/datasets/test_conv_catch.json'
    ...
    for i in range (num_iterations - num_over_iterations) :
        ...
        if (i + num_over_iterations + 1) % 1 == 0 :
            ...
            print("Cost after iteration {}: {}.".format(i + num_over_iterations + 1, g.getData('J')))
    print("Cost after iteration {}: {}.".format(num_iterations, g.getData('J')))
