...
10. 全连接操作
    ...
    > 此处的“全连接”与神经网络的全连接概念相同，表现形式不同

    全连接操作与子图相关。多个连通子图构成一个不连通的子图，当全连接操作符后跟着一个子图时，全连接操作符会将左操作数连接到子图的所有源点。如果一个子图后跟着一个全连接符，则子图的所有汇点会连接到右操作数。

    与层操作符`->`有本质不同的是，层操作符只会将前面块的最后一个汇点连接到下一个节点上，或者将当前点连接到后面块的源点上。

    当全连接操作符左右只有一个节点时，它的作用和`->`完全相同。

    * `X => ( x1 add y1 \n x2 add y2 )` : 全连接操作-分发，将`X`的值分发给`x1`和`x2`（`\n`是换行，表示里面有两个子块）
    * `( x1 add y1 \n x2 add y2 ) => Y` : 全连接操作-汇总，将`y1` 和`y2`的值汇总到`Y`上（`\n`是换行，表示里面有两个子块）
    * `X => Y` : 全连接操作-传递，与`X -> Y`相同

    全连接关键词一般都需要和特定的操作符一同使用，例如`X split(2) => (<block a> <block b>)`。这些特定的操作符可以把矩阵`X`拆成多个向量`x1, x2, x3,...`并向后传递，同样也有`(<block a> <block b>) => join(2) Y`，可以将子图中传递过来的向量`y1, y2, y3,...`合成一个矩阵`Y`来继续参与运算。
...
一个块由多个层组成，由层操作符`->`连接。层操作起连接并传递变量的作用，不严格分割层的逻辑结构。层操作符只将左表达式的最后一个汇点连接到右表达式的第一个源点，如果表达式存在多个源点或汇点，其余的将被忽略（相反，全连接操作符`=>`不会忽略）
