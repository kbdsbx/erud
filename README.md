...
2. 非页节点：存储操作和缓存
    ...
    * 复合一元操作
        ...
        * ...
    * 复合多元操作
        * L2_regularization（完成）:  L2正则化，对多个学习参数进行正则惩罚。L2正则化需要一个超参数lambda来表示惩罚率,调用方法为`L2_regularizatio(0.05) W1|W2|W3`
    * 带控制参数的一元或多元操作
...
3. 多参数运算符
    ...
    原始nous中所有运算符只有1~2个参数，称为左参和右参，使得搭建的图近似一个二叉树。但很多运算符需要多个参数，例如正则惩罚，需要W1, W2, W3, W4等。如果nous语言不能支持多参数运算符语法，就会极大限制nous的灵活性。

    多参语法与单参语法相同，左右参数都可以声明多个变量参与运算，且都为指向运算符的节点。运算符返回值不变，依旧为1个

    计算图本身支持大于1的节点入度，只需要正确实现运算符的前向传播和反向传播，此操作即可实现。

    * ``W1|W2|W3|W4 L2_regulation alpha`` 包含左右操作数的计算声明，左操作数为4个，用``|``分割，右操作数一个
...
3. 层操作
    ...
    * ``cast`` 或 ``=>``: 层多点连接符，运算时将左式子图的所有汇点的计算结果传递给右式子图的所有源点

    > v0.3 版本（rnn相关版本）新增

    层多点连接操作`=>`与子图相关。多个连通子图构成一个不连通的子图，当多点连接操作符后跟着一个子图时，多点连接操作符会将左操作数一一对应连接到子图的各个源点。如果一个子图后跟着一个多点连接操作符，则子图的所有汇点会一一对应连接到右操作数。

    与层操作符`->`有本质不同的是，层操作符只会将前面块的最后一个汇点连接到下一个节点上，或者将当前点连接到后面块的源点上。

    当多点连接操作符左右都只有一个节点时，它的作用和`->`完全相同。

    * `X scatter => ( x1 add y1 \n x2 add y2 )` : 多点连接-分发，将`X`的值通过`scatter`操作符分发给`x1`和`x2`（`\n`是换行，表示里面有两个子块）
    * `( x1 add y1 \n x2 add y2 ) gather => Y` : 多点连接-汇总，将`y1` 和`y2`的值通过`gather`操作符汇总到`Y`上（`\n`是换行，表示里面有两个子块）
    * `X => Y` : 多点连接操作-传递，与`X -> Y`相同

    多点连接关键词一般都需要和特定的操作符一同使用，例如`X scatter(2) => (<block a> <block b>)`。这些特定的操作符可以把矩阵`X`拆成多个向量`x1, x2, x3,...`并向后传递，同样也有`(<block a> <block b>) => gather(2) Y`，可以将子图中传递过来的向量`y1, y2, y3,...`合成一个矩阵`Y`来继续参与运算。

    <span style="color: red">具体的操作符还在设计中</span>
...
nous是一种结构化语言，能够快速实现各种计算图的搭建。nous解析器对语言的不同结构做了从上到下，从大到小的处理。

### 块控制

一个块有可能为以下几种情况

1. `<block>`，代码中没有任何块控制关键词`loop`等，没有块分割小括号`()`（小括号除了块分割以外还有其他用处），没有换行符号`\n`，整篇代码为一个块。此时块处理器会直接处理。
2. `<block> \n <block>`，带换行，不包含块分割用的小括号。这种情况块控制器会分成多个块，每个块皆由块处理器依次处理
3. `<block> opt (<block>) opt <block>`，这种情况块控制器会只剥离小括号，然后交给块处理器处理
2. `( <block> \n <block> \n <block> )`，小括号中包含多个行，以换行符`\n`分割，这种情况块控制器会剥离掉小括号，然后将每一行交给块处理器去处理

块控制器会处理一段包含多个块的语句，其处理依据为换行、小括号以及块控制关键词。块控制器会生成一个或多个连通子图，并返回所有子图的源点与汇点。

任何块都需要由块控制器预先处理并分割成子块，然后交给块处理器，考虑到语言的完备性，两者可以相互嵌套多次。

某些块控制运算可以包含参数，如循环生成`for t=1 to 5 (<block>)`（语法暂定），此时块控制会依据参数复制`t`次块语句，然后通过表达式替换的方式计算由尖括号`<>`声明的控制参数，如`x<t - 1>`会在第`1`句被替换为`x0`。


### 块处理

一个块如果只包含顺序处理的块，那么这个块将交由块处理器处理。此时的块处理器会顺序读取所有代码，依次分辨每一个由空格分割的完整元素的类型，并按照语法规则搭建计算图。

块处理器会处理一块语句，其不能包括换行。块处理器会生成一个连通子图，并返回连通子图的源点与汇点（一到多个）。

如果块处理过程中检测到块控制关键词或小括号（这里的小括号是结构上的小括号），就会递归调用块控制处理器，并按照子块前后的操作符（`->`或`=>`）来正确连接。

### 层操作

一个块由多个层组成，由层操作符`->`或`=>`连接。层操作起连接并传递变量的作用，不严格分割层的逻辑结构。层操作符`->`只将左表达式的最后一个汇点连接到右表达式的第一个源点，如果表达式存在多个源点或汇点，其余的将被忽略。多点连接操作符`=>`则会将左表达式的多个汇点一一对应地连接到右表达式的多个源点，如果数量不匹配，将会报错或忽略多余的点（待定）


### 表达式

一个层内的表达式为基础表达式，分为以下几种：

1. `opt`：只有一个操作符的为一元操作符，表示其等待上一个层传递参数，所以不能是块起始的层。
2. `opt Y`：二元操作符，有一个右操作数的同样等待上一层的参数。
3. `X opt Y`：二元操作符，可以是块的起始层。
4. `W1|W2|W3 opt`：多元操作符，操作数不定，由`|`符号分割，但本质上是将左操作数由单个张量改成了张量数组。其与`W1 opt W2|W3`、`W1|W2 opt W3`是等价的
5. `X opt1 Y opt2 Z`：有多个操作符合操作数组成的简短语句
6. `X opt1 (Y opt2 Z)`：如果表达式包含控制优先级的`()`，则说明层内包含子块，需要调用块控制器来处理
...
编写长期学习网络需要进行不间断的缓存操作，无论是防止计算机故障导致学习失败，还是使用“提前中止”正则化来寻找泛化最优解，都需要将整个计算图的相关信息多次存储在硬盘上，包括不限于
* 计算图结构
* 计算参数
* 已经运行的代价
* 其他

导出的数据为JSON格式，方便在学习框架的过程中随时检查整个计算图的状态，或者通过命名/匿名的方式将已经学习好的参数迁移到新的学习网络/测试网络中

### 像OSI一样层层存储

Erud的存储过程就像OSI或TCP/IP模型一样自上而下存储，自下而上还原，每一层都或多或少增加一些此层需要增加的信息，并在还原的时候读取出来

|层|作用|携带的信息|
|-|-|-|
|extend|导出的辅助方法|携带用户自定义信息<br>比如学习时的学习率，有效学习轮次，minibatch大小等信息|
|nous|解释器|code，即创建计算图的代码，用来还原整个计算图的结构|
|graph|计算图|无|
|node|计算图节点|code，此节点创建时的代码<br>ftimespend，上一次前向传播的时间<br> btimespend，上一次反向传播的时间<br>ftimetotal，前向传播总时间<br>btimetotal，反向传播总时间|
|payload|运算符、参数、样本等|名称、参数、运算必须的值等|
|updateable等|参数的更新方法等|移动平均动量等|


### 导出

```python
erud.exports(n = <nous object>, path = '<cache-path-file>', extends = { <something you want to save> })
```

### 导入
通过imports导入时会重新创建一个新的图实例

```python
n, extends = erud.imports(path = '<cache-path-file>')
```
...
2. `upf.momentum(rate, beta)`: 稀有的更新函数，使用加权平均进行偏差修正，能够增加梯度下降速度
3. `upf.adam(rate, beta_momentum, beta_rms)`: 史诗的更新函数，不仅使用加权平均进行偏差修正，还使用平方平均根对不同方向上的参数更新量进行调整，能大幅增加梯度下降速度（但也可能让梯度下降跳入局部最小值出不来）
